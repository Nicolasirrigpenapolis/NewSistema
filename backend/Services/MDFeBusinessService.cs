...
namespace Backend.Api.Services
{
    public class MDFeBusinessService : IMDFeBusinessService
    {
        ...
        /// <summary>
        /// Cria um novo MDF-e em branco no estado "Em Digitação"
        /// Retorna ID e dados básicos para edição no frontend
        /// </summary>
        public async Task<MDFe> CreateBlankMDFeAsync()
        {
            _logger.LogInformation("[MDFe] Criando novo MDF-e em branco");

            // Buscar o primeiro emitente ativo (ou criar lógica para pegar emitente do usuário logado)
            ...
            // Buscar MDFes existentes para gerar próximo número
            var mdfesExistentes = await _context.MDFes
            ...
            _logger.LogInformation("[MDFe] MDF-e ID {Id} criado em branco. Número: {Numero}, Série: {Serie}", 
        ...
        private async Task<MDFe> UpdateRascunhoAsync(int id, SalvarRascunhoDto rascunhoDto)
        {
            ...
            mdfe.UfIni = !string.IsNullOrWhiteSpace(rascunhoDto.UfIni)
                ? rascunhoDto.UfIni.Trim().ToUpper()
                : mdfe.UfIni; // Mantém valor existente se não informado
            mdfe.UfFim = !string.IsNullOrWhiteSpace(rascunhoDto.UfFim)
                ? rascunhoDto.UfFim.Trim().ToUpper()
                : mdfe.UfFim; // Mantém valor existente se não informado
